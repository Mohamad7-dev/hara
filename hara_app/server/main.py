...
def _seed():
    ...
    products = [
        ("p1", "user1", "محمد أبو أحمد", "زيت زيتون بلدي", "زيت زيتون عذراء طبيعي 100% معصور على البارد من مزرعة العائلة.", 35, "طعام", 50, 4.8, 23),
        ("p2", "user1", "محمد أبو أحمد", "تمر مجهول فاخر", "تمر مجهول عضوي فاخر، وزن 1 كجم، منتج طبيعي بدون مواد حافظة.", 28, "طعام", 30, 4.6, 15),
        ("p3", "user1", "محمد أبو أحمد", "عصير طبيعي", "عصير برتقال طبيعي طازج بدون سكر مضاف، معصور يومياً.", 8, "طعام", 100, 4.5, 8),
        ("p4", "user2", "سامي عوض", "شنطة يدوية", "شنطة يد مصنوعة يدوياً من القماش الفلسطيني المطرّز، تصميم عصري ومتين.", 45, "هدايا", 10, 5.0, 5),
        ("p5", "user2", "سامي عوض", "لابتوب مستعمل", "لابتوب بحالة ممتازة، معالج i5، رام 8 جيجا، شاشة 15 بوصة.", 850, "لابتوبات", 1, 4.3, 11),
        ("p6", "user1", "محمد أبو أحمد", "هاتف سامسونج", "هاتف سامسونج شبه جديد، بطارية ممتازة، مع الشاحن والعلبة.", 420, "هواتف", 1, 4.7, 18),
    ]
    for (pid, sid, sname, title, desc, price, cat, stock, rating, rcount) in products:
        conn.execute(
            "INSERT INTO products (id, seller_id, seller_name, title, description, price, currency, category, images, stock, featured, rating, rating_count, created_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)",
            (pid, sid, sname, title, desc, price, "شيكل", cat, "[]", stock, 1, rating, rcount, now),
        )

    posts = [
        ("post1", "user1", "محمد أبو أحمد", "بائع", "store", "عرض", "وصلت دفعة جديدة من زيت الزيتون البلدي 🍃 طلبك اليوم توصيل مجاني داخل الخليل.", True),
        ("post2", "user2", "رنا عودة", "مستخدمة", "person", "سؤال", "مين جرب التوصيل بالليل؟ سريع ولّا بطيء؟ وبكم التكلفة تقريباً من البيرة لرام الله؟", False),
        ("post3", "delivery1", "خالد حسن", "موصل", "motor", "تحديث", "جاهز اليوم للتوصيل السريع من الساعة 4 لمدة 10. المسافة الأقصى 15 كيلومتر. يسعدني خدمتكم.", True),
        ("post4", "user2", "سامي عوض", "بائع", "store", "عرض", "شنط يدوية مطرزة يدوياً بألوان مميزة 🎨 متوفرة بثلاث مقاسات. أول 5 طلبات بخصم 15%.", True),
        ("post5", "user1", "محمود جبارين", "مستخدم", "person", "مشكلة", "أحد المحلات ما أرسل طلبي من يومين 😟 مين عنده تجربة بيكيف بتتعاملوا مع الحالة هيك؟", False),
    ]
    for (pid, author_uid, author, role, avatar, cat, text, has_img) in posts:
        conn.execute(
            "INSERT INTO posts (id, author_uid, author, role, avatar_key, category, time, text, has_image) VALUES (?,?,?,?,?,?,?,?,?)",
            (pid, author_uid, author, role, avatar, cat, now, text, 1 if has_img else 0),
        )

    like_data = [
        ("post1", "user2"), ("post1", "delivery1"), ("post2", "user1"),
        ("post3", "user2"), ("post4", "user1"), ("post5", "delivery1"),
        ("post1", "admin1"),
    ]
    for pid, uid in like_data:
        conn.execute("INSERT OR IGNORE INTO post_likes (post_id, uid) VALUES (?,?)", (pid, uid))

    comments = [
        ("c1", "post", "post1", "user2", "رنا عودة", "مستخدمة", "person", "طيب النوعية بصراحة، اشتريت من عندكم بالسابق 😍", 0, None),
        ("r1", "post", "post1", "delivery1", "خالد حسن", "موصل", "motor", "أهلاً رنا، تسلمي على ثقتك 🙏 في في خدمتك بأي وقت.", 0, "c1"),
        ("c2", "post", "post1", "delivery1", "خالد حسن", "موصل", "motor", "التوصيل جاهز ينوصل بأي وقت قريب منك.", 0, None),
        ("c3", "post", "post1", "user2", "سامي عوض", "بائع", "store", "كم سعر التركة؟", 0, None),
        ("c4", "post", "post2", "user1", "محمد أبو أحمد", "بائع", "store", "جربت التوصيل بالليل، كان بحدود النص ساعة من البيرة لرام الله.", 0, None),
        ("c5", "post", "post2", "delivery1", "خالد حسن", "موصل", "motor", "التكلفة بالعادة بين 5 و 10 شيكل حسب المسافة.", 0, None),
        ("c6", "post", "post2", "user1", "محمود جبارين", "مستخدم", "person", "مشكلة بس أنه البعض ما بعمل بالليل، لازم تسأل قبل.", 0, None),
        ("c7", "post", "post3", "user2", "رنا عودة", "مستخدمة", "person", "بتمنى من الخليل تقدر توصّل؟", 0, None),
        ("r2", "post", "post3", "user2", "سامي عوض", "بائع", "store", "بنشحن للخليل كل أسبوع، أكيد نقدر نرتب معك.", 0, "c7"),
        ("r3", "post", "post3", "delivery1", "خالد حسن", "موصل", "motor", "لو بدك توصيل داخلي بالخليل، في موصلين عندنا هناك.", 0, "c7"),
        ("c8", "post", "post3", "user2", "سامي عوض", "بائع", "store", "ممتاز، عندي شحنة لرام الله بكرا.", 0, None),
        ("c9", "post", "post3", "delivery1", "خالد حسن", "موصل", "motor", "أهلاً رنا، حالياً الخليل برا النطاق بس ممكن نرتب موعد.", 0, None),
        ("c10", "post", "post4", "user1", "محمد أبو أحمد", "بائع", "store", "الألوان روعة، شو مقاسات المتوفرة؟", 0, None),
        ("c11", "post", "post4", "user2", "رنا عودة", "مستخدمة", "person", "أخذت وحدة بالخصم، التطريز نضيف جداً 👌", 0, None),
        ("c12", "post", "post5", "delivery1", "خالد حسن", "موصل", "motor", "غالباً في إمكانية إلغاء الطلب من الإعدادات، جرب.", 0, None),
        ("c13", "post", "post5", "user1", "محمد أبو أحمد", "بائع", "store", "أرسل اسم المحل ورقم الطلب وبنحاول نوصلك للدعم.", 0, None),
        ("c14", "post", "post5", "user2", "رنا عودة", "مستخدمة", "person", "حصلت معي نفس المشكلة مرة، انحلت لما تواصلت معهم بالرسائل.", 0, None),
        ("c15", "product", "p1", "user2", "رنا عودة", "مستخدمة", "person", "زيت نظيف وطعمه أصلي، أنصح فيه.", 5, None),
        ("r4", "product", "p1", "user2", "سامي عوض", "بائع", "store", "شكراً رنا، نطمّنك أنه الزيت من مزرعة عائلية.", 0, "c15"),
        ("c16", "product", "p1", "delivery1", "خالد حسن", "موصل", "motor", "التغليف ممتاز والتوصيل كان سريع.", 5, None),
        ("c17", "product", "p1", "user2", "سامي عوض", "بائع", "store", "منتج جيد، بس يا حيف ما كان في خصم للكمية.", 4, None),
        ("c18", "product", "p2", "user1", "محمود جبارين", "مستخدم", "person", "تمر فاخر فعلاً، ووزن الكيلو مظبوط.", 5, None),
        ("c19", "product", "p2", "user2", "رنا عودة", "مستخدمة", "person", "طعم ممتاز، ناقص نجمة عشان التغليف كان ممكن يكون أحسن.", 4, None),
        ("c20", "product", "p3", "user2", "سامي عوض", "بائع", "store", "عصير منعش وبدون سكر، تكرر عندي بالطلب.", 5, None),
        ("c21", "product", "p3", "user1", "محمد أبو أحمد", "بائع", "store", "جيد جداً، بس الكمية صغيرة بالعبوة.", 4, None),
        ("c22", "product", "p4", "user2", "رنا عودة", "مستخدمة", "person", "الشنتة تحفة، التطريز يدوي مبدع.", 5, None),
        ("c23", "product", "p4", "user1", "محمود جبارين", "مستخدم", "person", "هدية رائعة، وصلت مغلفة بشكل جميل.", 5, None),
        ("c24", "product", "p5", "delivery1", "خالد حسن", "موصل", "motor", "الجهاز بحالة ممتازة، سعر مناسب.", 4, None),
        ("c25", "product", "p5", "user2", "سامي عوض", "بائع", "store", "المفاوضة سهلة والبائع محترم.", 5, None),
        ("c26", "product", "p6", "user1", "محمد أبو أحمد", "بائع", "store", "الهاتف شبه جديد والبطارية قوية.", 5, None),
        ("c27", "product", "p6", "user2", "رنا عودة", "مستخدمة", "person", "ممتاز، بس ما كان في ضمان.", 4, None),
        ("c28", "product", "p6", "delivery1", "خالد حسن", "موصل", "motor", "وصلني بسرعة وكان مغلف كويس.", 5, None),
    ]
    for (cid, kind, target, author_uid, author, role, avatar, text, rating, parent) in comments:
        conn.execute(
            "INSERT INTO comments (id, kind, target_id, author_uid, author, role, avatar_key, text, rating, time, parent_id) VALUES (?,?,?,?,?,?,?,?,?,?,?)",
            (cid, kind, target, author_uid, author, role, avatar, text, rating, now, parent),
        )

    _open_conversation(conn, "user1", conn.execute("SELECT * FROM users WHERE uid='user2'").fetchone())

    _add_notification(conn, "user1", "order", "طلب جديد", "طلبك رقم #1042 أصبح قيد التوصيل")
    _add_notification(conn, "user1", "message", "رسالة جديدة", "سامي عوض: أهلاً بك! نعم الشنطة متوفرة.")
